...
const ManualsMenu = () => {
    ...
    const handleCreate = async () => {
        ...
        try {
            ...
            const response = await uploadManual(file); // Aquí pasamos solo el archivo
            if (response) {
                fetchManuals(page); // Volver a cargar las imágenes en la misma página
                    ...
                    >
                        <div className="w-full h-full flex items-center justify-center">
                            <div className="bg-white p-4 rounded-lg w-[30%] space-y-3">
                                <h1 className="font-bold text-base">{selectedFile?.name}</h1>
                                <img
